...
RUTA_PRUEBAS = Path("pruebas_volumen")
VALORES_K=Path("pruebas_volumen/k_por_grafo_volumen.json")
...
def main():
    ...
    for archivo in RUTA_PRUEBAS.iterdir():
        ...
        cat = obtener_categoria(archivo.name)  # saca '.txt'
    

        k = datos[cat]
        t = medir_tiempo(archivo, k)
        tiempos_por_categoria[cat].append(t)
        print(f"{cat:25s}: {t:.4f}s")
    ...
    plt.title(f"Tiempo de ejecución con Algoritmo de Aproximación")
    plt.xticks(rotation=45)
    plt.grid(axis='y', linestyle='--', alpha=0.6)
    plt.tight_layout()
    os.makedirs("graficos", exist_ok=True)
    plt.savefig(f"graficos/tiempo_por_test_aproximacion.png")
    plt.show()
